...
Completed:
- Application ID, namespace, and internal package aligned on `org.openbabyphone`
- NSD service name no longer exposes the Android device model
- README, privacy policy, security policy, store metadata, and roadmap aligned with
  the local-only product direction
- Pairing code auto-generated by default via `PairingCodeGenerator`
- Empty pairing code is no longer the normal configuration
- Protocol versioning and capability negotiation implemented (OBP2 handshake)

Remaining beta-blockers:
- Rename NSD service type from `_childmonitor._tcp.` to `_openbabyphone._tcp.` (#13)
- Move manual address entry to an explicit advanced path (#54)
- Add user-configurable device name for NSD discovery (#58)
