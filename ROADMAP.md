...
## 1. Open Beta Basis (Completed)

Goal: make the fork coherent, safe by default, and honest in its public materials.

All beta-blockers resolved:
- Application ID, namespace, and internal package aligned on `org.openbabyphone` (#21)
- NSD service name no longer exposes the Android device model (#23)
- README, privacy policy, security policy, store metadata, and roadmap aligned with
  the local-only product direction (#26)
- Pairing code auto-generated by default via `PairingCodeGenerator` (#27)
- Empty pairing code is no longer the normal configuration
- Protocol versioning and capability negotiation implemented (OBP2 handshake) (#25)
- NSD service type renamed from `_childmonitor._tcp.` to `_openbabyphone._tcp.` (#13)
- Manual address entry moved to an explicit advanced path (#54)
- User-configurable device name for NSD discovery (#58)

## 2. Reliability Release (Active)

Goal: prove the app can run through the night on real devices.

Most bug-fix backlog items from the immediate risk backlog are resolved. The
remaining work is real-device validation and platform-behavior hardening.
...
The child device has one microphone pipeline and fans out encoded audio
frames to up to 5 connected parent devices via `ClientManager`. The lifecycle
around connect, disconnect, max-clients, and runtime pairing changes has been
hardened.

Completed:
- Server socket lifecycle stable when max clients are reached (#19, #31)
- NSD re-registration when capacity becomes available (#19)
- ClientManager callback for client count changes and removals
- Child UI updates reliably on connect, disconnect, and slow-client drops
- Busy-loop fix when max clients reached (#31)
- AEAD nonce reuse for multi-parent auth fixed (#47)
- Pairing code changes during monitoring handled safely (#30)
- Integration tests for multi-parent scenarios (#51)
- Multi-parent support documented in README and UI (#52)
