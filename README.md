...
Up to 5 parent devices can listen to the same child device simultaneously on
the local network. The child device fans out encoded audio frames to all
connected parents through a bounded per-client queue. When a parent is too slow
to keep up (e.g. poor Wi-Fi), the child device drops that parent gracefully
instead of blocking other parents. When a parent disconnects, the child device
re-registers itself via NSD so new parents can discover it again.
...
- Challenge-response authentication with Argon2i key derivation
...
- Multi-parent audio fan-out from one microphone recording pipeline (up to 5)
...
2. The child device generates a strong pairing code by default and displays it.
...
6. Enter the pairing code shown on the child device to enable authentication and
   encrypted transport.

The child and parent negotiate protocol version and codec capabilities during
the handshake. If the parent's protocol version is incompatible, the connection
is rejected with a clear error instead of producing garbled audio.

An empty pairing code disables authentication and encryption. This is possible
but not the default. Use a non-empty pairing code for normal operation.
...
- A non-empty pairing code enables challenge-response authentication with Argon2i key derivation.
- A non-empty pairing code also enables ChaCha20-Poly1305 encrypted audio frames.
- An empty pairing code means open local connections and unencrypted audio; this is possible but not the default.
- The child and parent negotiate protocol version and codec capabilities during the handshake.
