...
- Lint aborts on errors; `MissingTranslation` is downgraded to a warning in `app/build.gradle`. `GradleDependency` warnings are disabled until #83 (Kotlin 2.x upgrade) is resolved.
...
- Gradle `defaultConfig` has the effective release version (`versionCode 18`, `versionName "1.1.0-alpha.3"`); the manifest does not define version fields.
