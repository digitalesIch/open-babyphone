...
- Settings are implemented by the Compose `SettingsScreen` inside `MainActivity`'s navigation graph; there is no separate settings activity.
...
- Navigation uses type-safe `@Serializable` routes defined in `navigation/NavRoutes.kt` and wired in `MainActivity.kt` via `NavHost`. To add a screen, add a route object/data class in `NavRoutes.kt` and a `composable<Route>` block in `MainActivity.kt`. Current routes: `Start`, `Monitor`, `Settings`, `Discover`, `DiscoverAddress`, `DiscoverWifiDirect`, `ConnectionHelp`, `Listen`.
- Listen notifications resume through the non-exported `ListenResumeActivity`; there is no externally addressable listen deep link. The trampoline creates a process-local request for a trusted identity before opening `MainActivity`.
...
- Adaptive/tablet strategy is responsive single-pane: screens cap content width with `Box` + `widthIn(max = 600.dp)`. No multi-pane layout (Navigation 3 remains deferred). Official Compose Preview Screenshot Testing is enabled with plugin `0.0.1-alpha15`; previews are under `app/src/screenshotTest/kotlin` and references under `app/src/screenshotTestDebug/reference`.
- Audio/networking is service-driven: `MonitorService` advertises `_openbabyphone._tcp.` with Android NSD, binds from TCP port `10000` upward, requires OBP4 mutual authentication with a persistent alphanumeric pairing code, records mic audio, and streams encrypted G.711 u-law frames; `ListenService` performs the parent-side handshake, validates and decodes frames, and plays the stream. `MicrophoneSensitivity` applies software gain to PCM samples before encoding; levels are Normal, High, Very high.
...
- Trusted child pairing is implemented via the application-scoped `TrustedChildStore` and `ChildDeviceIdentityStore`. Trusted profile metadata excludes the pairing code; parent credentials are stored separately under Android Keystore AES-GCM protection and are committed only after successful OBP4 authentication. The QR code uses `PairingQrCode` to encode a structured payload with child ID, pairing ID, and pairing code, while pending first-pair material remains as wipeable mutable data in a bounded process store. Parents remember known children and reconnect without re-scanning. The child pairing code and pairing generation are committed atomically in the same SharedPreferences file.
...
- Gradle wrapper is 9.6.1; Android Gradle Plugin is 9.3.1; Kotlin is 2.4.10; Compose BOM is 2026.06.01; Compose Compiler is configured through the Kotlin Compose compiler Gradle plugin; navigation-compose is 2.9.8.
...
- Local JVM/Robolectric tests live under `app/src/test/kotlin`; Android instrumentation and Compose behavior/configuration tests live under `app/src/androidTest/kotlin`. Run instrumentation with `./gradlew connectedDebugAndroidTest`. Screenshot previews live under `app/src/screenshotTest/kotlin`; generate baselines with `./gradlew updateDebugScreenshotTest` and validate with `./gradlew validateDebugScreenshotTest`.
- Screenshot references live under `app/src/screenshotTestDebug/reference`. Compose Preview Screenshot Testing is experimental and LayoutLib-based; camera, notifications, foreground services, audio hardware, OEM power behavior, and overnight reliability remain checks on the maintainer's available devices as documented in `docs/testing.md`.
...
- SHA-256 dependency verification metadata is committed at `gradle/verification-metadata.xml`; normal and CI builds use strict verification. Android variant lockfiles are intentionally not used. Reviewed release checklists and the external fdroiddata template live under `docs/release/`.
...
- This is a no-budget project with one current developer. Do not make paid services, external usability panels, purchased test hardware, or broad OEM/device fleets release requirements. Prefer automation plus honest, risk-based checks on the two or three phones available to the maintainer.
