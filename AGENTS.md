...
- Adaptive/tablet strategy is responsive single-pane: screens cap content width with `Box` + `widthIn(max = 600.dp)`. No multi-pane layout (deferred; compileSdk 34 and no Navigation 3). Screenshot tests are deferred until AGP 8.5+ is adopted (the Compose preview screenshot plugin requires AGP 8.5+).
...
- Gradle wrapper is 8.6; Android Gradle Plugin is 8.2.2; Kotlin is 1.9.25; Compose BOM is 2024.09.03; Compose compiler extension is 1.5.15; navigation-compose is 2.8.9.
